...
import com.sexyuncle.springboot.scp.vo.PurchaseOrderPurchaserVO;
...
public class PurchaseReturnService {
	...
	public AtomicLongArray uploadPurchaseReturns(MultipartFile file, ReturnState returnState) {
		try {
			...
			List<PurchaseOrderPurchaserVO> purchasers = sqlOperations.namedSqlQuery("findAllPurchasers", PurchaseOrderPurchaserVO.class);
			Map<Long, String> orderIdPurchaserMap = purchasers.stream().collect(toMap(PurchaseOrderPurchaserVO::getOrderId, PurchaseOrderPurchaserVO::getPurchaser));
			while (iterator.hasNext()) {
				...
				returnItems.add(toReturnItem(row, cityMap, orderIdPurchaserMap));
			}
	...
	public ReturnItem toReturnItem(Row row, Map<Long, String> orderIdCityMap, Map<Long, String> orderIdPurchaserMap) {
		//ReturnItem, Sku
		int returnIdIndex = 0;
		int skuCodeIndex = 3;
		int skuNameIndex = 4;
		...
		returnItem.setPurchaser(orderIdPurchaserMap.get(returnItem.getOrderId()));
		...
		returnItem.setApplyDate(ExcelUtils.dateTimeVal(row, applyDateIndex));
	...
	public List<PurchaseReturnSearchResultVO> searchPurchaseReturn(PurchaseReturnSearchVO purchaseReturnSearchVO, Page page) {
		Map<String, Object> params = new HashMap<>();
		if (purchaseReturnSearchVO.getReturnId() != null) {
			params.put("returnId", purchaseReturnSearchVO.getReturnId());
		}
		params.put("applyDateBegin", purchaseReturnSearchVO.getApplyDateBegin());
		params.put("applyDateEnd", purchaseReturnSearchVO.getApplyDateEnd());
		if (isNotBlank(purchaseReturnSearchVO.getCity())) {
			params.put("city", QueryUtils.innerMatch(purchaseReturnSearchVO.getCity()));
		}
		if (purchaseReturnSearchVO.getState() != null) {
			params.put("state", purchaseReturnSearchVO.getState().getCode());
		}
		params.put("purchaser", QueryUtils.innerMatch(purchaseReturnSearchVO.getPurchaser()));

		return sqlOperations.namedSqlQuery("queryPurchaserReturns", params, PurchaseReturnSearchResultVO.class, page);
	}
